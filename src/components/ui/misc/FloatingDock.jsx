    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 flex flex-col items-center gap-2">
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
                ...
                <a
                  ...
                  <div className="grid h-6 w-6 place-items-center [&_svg]:h-full [&_svg]:w-full">{item.icon}</div>
                </a>
              </motion.div>
            ))}
          </motion.div>
